...
class StripMBConv(nn.Module):
    def __init__(self,
        ...
        super(StripMBConv, self).__init__()
        ...
        self.dwconv = ConvBNAct(expanded_c,
                                ...
                                norm_layer=norm_layer,
                                activation_layer=activation_layer)
        self.bwconv = ConvBNAct(expanded_c,
                                expanded_c,
                                kernel_size=(5, 1),
                                stride=stride,
                                groups=expanded_c ,
    ...
    def forward(self, x: Tensor) -> Tensor:
        result_1 = self.expand_conv(x)
        result_2 = self.dwconv(result_1)
        result_3 = self.bwconv(result_1)
...
class StripFusedMBConv(nn.Module):
    def __init__(self,
                 kernel_size,
                 input_c: int,
                 out_c: int,
                 ...
                 drop_rate: float,
                 norm_layer: Callable[..., nn.Module]):
        super(StripFusedMBConv, self).__init__()
        ...
        if self.has_expansion:
            ...
            self.expand_bconv = ConvBNAct(input_c,
                                         expanded_c,
                                         kernel_size=(5, 1),
                                         stride=stride,
                                         norm_layer=norm_layer,
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.has_expansion:
            result_1 = self.expand_conv(x)
            result_2 = self.expand_bconv(x)
            result = result_2 + result_1
            result = self.se(result)
            result = self.project_conv(result)
        else:
            result_1 = self.project_conv(x)
...
def LaserNet(num_classes: int = 1000):
    ...
    model_config = [
                    [3, (1,5), 2, 4, 24, 48, 2, 0.25],
                    [3, (1,5), 2, 4, 48, 64, 2, 0.25],
                    [9, (1,5), 2, 4, 64, 128, 1, 0.25],
                    [3, (1,5), 2, 4, 128, 256, 1, 0.25]]

    model = LaserNet(model_cnf=model_config,
